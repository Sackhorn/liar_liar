...
class CarliniWagner(Attack):
    ...
    def run_attack(model, data_sample, target_class):
        ...
        iter_max = 10000
        image = c_and_w(image, model, target_label, perturbation, optimizer, iter_max)
        return (image, model(image), 0) #TODO: fix the returns here to be consistent with run_attacks
...
def test_c_and_w():
    ...
    for data_sample in  model.get_dataset(Split.TEST, batch_size=batch_size, shuffle=1).take(1):
        ret_image, ret_labels, _ = CarliniWagner.run_attack(model, data_sample, 1)
...
if __name__ == "__main__":
    start = time.time()
    test_c_and_w()
    print("CW took: " + str(time.time() - start))
